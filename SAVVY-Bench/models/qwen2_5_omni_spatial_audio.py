"""Qwen2.5-Omni with shared-encoder stereo audio for SAVVY-Bench.

The stock Qwen2.5-Omni processor/model treats every waveform as one logical
audio stream. This adapter keeps a stereo WAV as two streams, applies the same
pretrained AudioEncoder to the left and right channels independently, and
concatenates their output token sequences as [left tokens, right tokens].
...
def _merge_stereo_feature_lengths(feature_lengths: torch.Tensor) -> torch.Tensor:
    """Represent two channel token lengths as one logical audio length.
    ...
    ``left_tokens + right_tokens`` positions.
    """
    if feature_lengths.numel() % STEREO_CHANNELS:
        raise ValueError(
            "Stereo audio lengths must contain exactly two entries per video; "
            f"got {feature_lengths.numel()} entries."
        )

    per_channel = feature_lengths.reshape(-1, STEREO_CHANNELS)
    merged_token_lengths = _audio_output_lengths(per_channel).sum(dim=1)
    return merged_token_lengths * 4


class Qwen2_5OmniSpatialAudioProcessor(Qwen2_5OmniProcessor):
    """Create one logical placeholder sequence from each stereo pair."""
    ...
    def replace_multimodal_special_tokens(
        ...
        if audio_lengths:
            if len(audio_lengths) % STEREO_CHANNELS:
                raise ValueError(
                    "The processor expects two audio channel arrays per video; "
                    f"got {len(audio_lengths)} arrays."
                )
            audio_lengths = [
                sum(audio_lengths[index : index + STEREO_CHANNELS])
                for index in range(0, len(audio_lengths), STEREO_CHANNELS)
            ]
...
class Qwen2_5OmniSpatialAudioThinker(Qwen2_5OmniThinkerForConditionalGeneration):
    """Reuse one AudioEncoder for both channels, then concatenate its tokens."""
    ...
    def get_audio_features(
        ...
        **kwargs,
        ...
        if input_features.shape[0] % STEREO_CHANNELS:
            raise ValueError(
                "input_features must contain left/right pairs in its batch "
                f"dimension; got shape {tuple(input_features.shape)}."
            )

        # Both calls use self.audio_tower, hence exactly the same pretrained
        ...
        # masked_scatter consumes features in this flattened pair order:
        # video1-left, video1-right, video2-left, video2-right, ...
    ...
    def get_rope_index(
        ...
        if audio_seqlens is not None:
            audio_seqlens = _merge_stereo_feature_lengths(audio_seqlens)
...
class Qwen2_5OmniSpatialAudioForConditionalGeneration(
    Qwen2_5OmniForConditionalGeneration
):
    """Full Qwen2.5-Omni model using the stereo-aware Thinker."""
    ...
    def __init__(self, config):
        ...
        # Thinker before allocating the stereo-aware replacement.
        Qwen2_5OmniPreTrainedModel.__init__(self, config)
        self.thinker = Qwen2_5OmniSpatialAudioThinker(config.thinker_config)

        ###### For testing ######
        text_model = self.thinker.model
        if not hasattr(text_model, "norm"):
            raise AttributeError(
                "Qwen2_5OmniThinkerTextModel.norm was not found; cannot attach "
                "the final-layer output shape hook."
            )

        def _print_final_norm_output_shape(module, inputs, output):
            norm_output = output[0] if isinstance(output, (tuple, list)) else output
            shape = getattr(norm_output, "shape", None)
            print(
                f"[Qwen2.5-Omni] Qwen2_5OmniThinkerTextModel.norm output shape: "
                f"{tuple(shape) if shape is not None else type(norm_output).__name__}",
                flush=True,
            )

        self._final_norm_shape_hook = text_model.norm.register_forward_hook(
            _print_final_norm_output_shape
        )

        if not hasattr(self.thinker, "lm_head"):
            raise AttributeError(
                "Qwen2_5OmniThinkerForConditionalGeneration.lm_head was not "
                "found; cannot attach the input feature shape hook."
            )

        def _print_lm_head_input_shape(module, inputs):
            lm_head_input = inputs[0] if inputs else None
            shape = getattr(lm_head_input, "shape", None)
            print(
                "[Qwen2.5-Omni] Qwen2_5OmniThinkerForConditionalGeneration."
                "lm_head input shape: "
                f"{tuple(shape) if shape is not None else type(lm_head_input).__name__}",
                flush=True,
            )

        self._lm_head_input_shape_hook = self.thinker.lm_head.register_forward_pre_hook(
            _print_lm_head_input_shape
        )
        ###### For testing ######
...
@register_model("qwen2_5_omni_spatial_audio")
class Qwen2_5_Omni_Spatial(lmms):
    """SAVVY adapter using aligned silent video and a stereo WAV."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._model = Qwen2_5OmniSpatialAudioForConditionalGeneration.from_pretrained(
            pretrained,
        ...
        self.processor = Qwen2_5OmniSpatialAudioProcessor.from_pretrained(
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        for request in pbar:
            ...
            # above as two independent channel arrays.
            _, images, videos = process_mm_info(
                conversation, use_audio_in_video=False
            )
            # print('*'*15)
            # print(type(videos), len(videos), type(videos[0]),'videos[].shape = '+str(videos[0].shape))
            # print('*'*15)
            inputs = self.processor(
                text=text,
                audio=stereo_channels,
                images=images,
                videos=videos,
                return_tensors="pt",
                padding=True,
